...
def render_login_page():
    ...
    with col2:
        role_tab1, role_tab2 = st.tabs([
            f"🏫 {T.get('faculty_tab', 'Faculty Login')}",
            f"🎓 {T.get('student_tab', 'Student Login')}"
        ])

        # ==================== FACULTY LOGIN ====================
        with role_tab1:
            st.markdown(f"""
            <div class="teacher-header" style="text-align:center; margin-top: 10px; margin-bottom: 10px;">{T["login_portal_text"]}</div>
            """, unsafe_allow_html=True)
        ...
        with role_tab2:
            st.markdown(f"""
            <div class="teacher-header" style="text-align:center; margin-top: 10px; margin-bottom: 10px;">
                {T.get("student_portal_text", "Student Portal")}
            </div>
            """, unsafe_allow_html=True)

            # Student icon and description
            st.markdown("""
            <div style="text-align:center; margin-bottom: 20px;">
                <div style="font-size: 48px; margin-bottom: 8px;">🎓</div>
                <div style="color: #A0AEC0; font-size: 14px; font-family: 'Plus Jakarta Sans', sans-serif;">
