...
from data_pro    import run_pipeline, get_summary_stats, get_at_risk_students
...
def render_home_page():
    ...
    _kpi(c6, f"{(df['semester_marks']>=120).mean()*100:.1f}%", "Pass Rate",  "marks ≥ 120",        "#1a5276")
    ...
    with col4:
        st.markdown('<div class="sh">Attendance vs semester marks (300 sample)</div>', unsafe_allow_html=True)
        fig4 = px.scatter(df.sample(300, random_state=1),
